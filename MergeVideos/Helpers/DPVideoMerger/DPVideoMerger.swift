...
////                let videoCustomizedSize = CGSize(width: height, height: width)
////                mutableVideoComposition.renderSize = videoCustomizedSize
////            }
////            else {
...
//                    case .completed:
//                        debugPrint("Successfully merged: %@", strFilePath)
//                        exportCompletion()
//                    case .failed:
//                        debugPrint("Failed")
//                        exportCompletion()
...
//
//        let strVideoPath = UserDefaults.standard.value(forKey: "strVideoPath") as! String
//        return strVideoPath
//
        ...
        for  videoFileURL in videoFileURLs {
            ...
            do {
                ...
                case UIImage.Orientation.right:
                    ...
                    layerInstruction.setTransform(transform.concatenating(Scale.concatenating(Move)), at: kCMTimeZero)
...
//                layerInstruction.setTransform(Scale.concatenating(Move), at: CMTime.zero)
    ...
    open func gridMergeVideos(withFileURLs
        ...
        for i in 0 ..< videoFileURLs.count {
            ...
            do {
                try videoTrack.insertTimeRange(CMTimeRangeMake(kCMTimeZero, maxTime), of: videoAsset, at: kCMTimeZero)
            } catch {
                DispatchQueue.main.async { completion(nil,error) }
                return
            }
...
fileprivate extension DPVideoMerger {
    ...
    func insertVideoWithTransform(_ isRepeatVideo: Bool, _ subInstruction: AVMutableVideoCompositionLayerInstruction, _ Scale: CGAffineTransform, _ Move: CGAffineTransform, _ arrAVMutableVideoCompositionLayerInstruction: inout [AVMutableVideoCompositionLayerInstruction], _ asset: AVURLAsset, _ composition: AVMutableComposition, _ completion: @escaping (URL?, Error?) -> Void, _ maxTime: CMTime) -> Bool {
        if (isRepeatVideo) {
            subInstruction.setTransform(Scale.concatenating(Move), at: kCMTimeZero)
            ...
            repeat {
                ...
                if CMTimeCompare(maxTime, atTime) != 0 {
                    ...
                    do {
                        try videoTrack.insertTimeRange(CMTimeRangeMake(kCMTimeZero, vDuration), of: asset.tracks(withMediaType: .video)[0], at: atTime)
                    } catch {
                        DispatchQueue.main.async { completion(nil,error) }
                        return false
                    }
    ...
    func addAudioToMergedVideo(_ audioFileURL: URL?, _ composition: AVMutableComposition, _ isRepeatAudio : Bool , _ maxTime: CMTime, _ completion: @escaping (URL?, Error?) -> Void) {
        if let audioFileURL = audioFileURL {
            ...
            do {
                try audioTrack.insertTimeRange(CMTimeRangeMake(kCMTimeZero, maxTime.seconds < asset.duration.seconds ? maxTime : asset.duration), of: audioAsset, at: kCMTimeZero)
            } catch {
                DispatchQueue.main.async { completion(nil,error) }
                return
            }
            if (isRepeatAudio && maxTime.seconds > asset.duration.seconds) {
                ...
                repeat {
                    ...
                    if CMTimeCompare(maxTime, atTime) != 0 {
                        ...
                        do {
                            try audioTrack.insertTimeRange(CMTimeRangeMake(kCMTimeZero, aDuration), of: audioAsset, at: atTime)
                        } catch {
                            DispatchQueue.main.async { completion(nil,error) }
                            return
                        }
    ...
    func exportMergedVideo(_ instructions: [AVVideoCompositionInstructionProtocol], _ highestFrameRate: Int, _ videoResolution: CGSize, _ composition: AVMutableComposition, _ videoQuality: String, _ completion: @escaping (URL?, Error?) -> Void) {
        ...
        mainComposition.frameDuration = CMTimeMake(1, Int32(highestFrameRate))
