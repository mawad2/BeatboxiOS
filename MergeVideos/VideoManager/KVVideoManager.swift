...
class KVVideoManager: NSObject {
    ...
    func mergeCustom(arrayVideos:[AVAsset] , isFinalVideo :Bool , completion:@escaping Completion) -> Void {
    doCustomMerge(arrayVideos: arrayVideos, isFinal: isFinalVideo, animation: false, completion: completion)
    }
    ...
    private func doCustomMerge(arrayVideos:[AVAsset], isFinal : Bool , animation:Bool, completion:@escaping Completion) -> Void {
        var insertTime = kCMTimeZero
        var arrayLayerInstructions:[AVMutableVideoCompositionLayerInstruction] = []
        var outputSize = CGSize.init(width: 0, height: 0)
    ...
    func makeVideoFrom(data:[VideoData], textData:[TextData]?, completion:@escaping Completion) -> Void {
        ...
        guard let bgVideoURL = Bundle.main.url(forResource: "black", withExtension: "mov") else {
            debugPrint("Need black background video !")
            completion(nil,nil)
            return
        }
        ...
        for videoData in data {
            if videoData.isVideo {
                ...
                do {
                    ...
                    debugPrint("Load background track error")
                }
    ...
    func cropVideo(sourceURL: URL, startTime: Double, endTime: Double, completion: @escaping (_ outputUrl:  URL) -> Void)
    {
        ...
        if  length == 0 {
            completion(URL.init(string: "file://") ?? URL.init(string: "")!)
            return
        }
        
        var outputURL = documentDirectory.appendingPathComponent("CropedTemp")
        do {
            try fileManager.createDirectory(at: outputURL, withIntermediateDirectories: true, attributes: nil)
            let ticks = Date().ticks
            outputURL = outputURL.appendingPathComponent("\(ticks)\(sourceURL.deletingPathExtension().lastPathComponent).mp4")
        ...
        guard let exportSession = AVAssetExportSession(asset: asset, presetName: setVideoQualityForExporter) else { return }
        ...
        //            case .loaded:
        //                let length : Float64 = CMTimeGetSeconds(asset.duration);
        //                debugPrint("video length: \(time) seconds")
        //
        //                break;
        //            case .unknown:
        //                debugPrint("failed")
